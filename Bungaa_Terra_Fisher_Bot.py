...
from Bungaa_Terra_utils import get_terraria_window, background_click, get_window_pos_mouse, get_cropped_screenshot, mean_squared_error
...
class main_programm():
    ...
    def __init__(self):
        ...
        self.terraria_window = get_terraria_window()
    ...
    def start_fisher(self, border_len):
        ...
        fish_counter = 0
        sleep(2)
        old_screenshot = get_cropped_screenshot(
            self.terraria_window, float_x, float_y, border_len)
        max_error = 0

        while True:
            if self.quit_flag:
                sys.exit()
            current_screenshot = get_cropped_screenshot(
                self.terraria_window, float_x, float_y, border_len)
            error, diff = mean_squared_error(
            ...
            if error > 39.1:
                ...
                # background click through win32api not working for terraria
                # background_click(self.terraria_window, float_x, float_y)
                mouse.press(mouse.Button.left)
                sleep(0.2)
                # background_click(self.terraria_window, throw_x, throw_y)
                mouse.press(mouse.Button.left)
                sleep(1.5)
            sleep(0.2)
            old_screenshot = current_screenshot
    ...
    def on_mouse_press(self, mouse_x, mouse_y, button, pressed):
        ...
        if button == mouse.Button.left:
            self.throw_point = get_window_pos_mouse(self.terraria_window, mouse_x, mouse_y)
            print("left")
            if self.float_point:
                return False
        elif button == mouse.Button.right:
            self.float_point = get_window_pos_mouse(self.terraria_window, mouse_x, mouse_y)
            print('right')
            if self.throw_point:
                return False
